...
export function MultiSelect({
  ...
  function toggleValue(value: string) {
    ...
    const nextSet = getNewSet(selectedValues);
...
export function MultiSelectValue({
  ...
  const checkOverflow = useCallback(() => {
    ...
    const itemsEls = containerElement.querySelectorAll<HTMLElement>(
      '[data-selected-item]',
    );

    if (overflowElement) overflowElement.style.display = 'none';

    itemsEls.forEach((child) => child.style.removeProperty('display'));

    let amount = 0;

    for (let i = itemsEls.length - 1; i >= 0; i--) {
      const child = itemsEls[i]!;

      if (containerElement.scrollWidth <= containerElement.clientWidth) break;

      amount = itemsEls.length - i;
      ...
      {[...selectedValues]
        ...
        .map((value) => (
          <Badge
            ...
            data-selected-item
            className={cn('group flex items-center gap-1', badgeClassName)}
            onClick={
              clickToRemove
                ? (e) => {
                    e.stopPropagation();
...
export function MultiSelectItem({
  ...
  }, [value, children, badgeLabel, onItemAdded]);
